...
# 3. 数据预处理主函数
def preprocess_data(train_path, selected_features):
    # 加载数据
    print("加载数据...")
    data = load_data_csv(train_path)

    # 筛选特征并处理
    X_test_raw = data[selected_features]
    y_test = data['label']
    ...
    X_test_normalized = minmax_scaler.fit_transform(X_test_raw)
    # 第二步：标准化处理
    ...
    X_test_cnn = reshape_to_cnn_format(X_test_processed)

    return X_test_cnn, y_test